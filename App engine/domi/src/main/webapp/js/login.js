	...
	gapi.client.domi.consultaClientes().execute(function(resp){
		 if (!resp.code) {
           ...
           for (var i=0;i<resp.items.length;i++) {
        	   ...
        	   result =  result+ resp.items[i].idCliente +"  "+resp.items[i].correoCliente +" "+resp.items[i].passwordCliente +" <br>";
           if(resp.items[i].correoCliente == username && resp.items[i].passwordCliente == password ){
        	   int=1;
